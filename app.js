...
try{
 ...
 if(saved&&[8,16,24].includes(saved.count)&&Array.isArray(saved.colors)&&saved.colors.length===saved.count&&saved.colors.every(isColor)&&Array.isArray(saved.palette)&&saved.palette.length===4&&saved.palette.every(isColor)){
  state={count:saved.count,colors:[...saved.colors],palette:[...saved.palette],
...
// 24 threads use twelve evenly distributed pairs, with a 48-slot disk.
function slotCount(){return state.count===24?48:32;}
...
// Rounded, overlapping scales arranged in offset rows for a continuous band.
// Each preview row shows half the threads; fixed scale size preserves proportional width.
const columns=state.count/2,segmentWidth=10,segmentHeight=13,rows=52;
const columnStep=7.5,rowStep=8;
...
const d=`M${x} ${y} a${rx} ${ry} 0 1 1 0 ${segmentHeight} a${rx} ${ry} 0 1 1 0 ${-segmentHeight} Z`;
...
$('custom').oninput=e=>{state.selected=e.target.value;render();};
